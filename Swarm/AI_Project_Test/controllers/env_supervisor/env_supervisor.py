...
class CleaningSupervisor(Supervisor):
    def __init__(self):
        ...
        self.episode_steps = [[] for _ in range(self.num_robots)]
    ...
    def save_results(self):
        with open('results.csv', 'w', newline='') as f:
            ...
            header = ['Episode']
            for i in range(self.num_robots):
                header.extend([f'Robot{i+1}_Reward', f'Robot{i+1}_Cleaned', f'Robot{i+1}_Steps'])
            writer.writerow(header)
            for ep in range(len(self.reward_histories[0])):
                row = [ep]
                for i in range(self.num_robots):
                    row.extend([self.reward_histories[i][ep], self.cleaned_histories[i][ep], self.episode_steps[i][ep]])
                writer.writerow(row)
    ...
    def plot_results(self):
        ...
        for i in range(self.num_robots):
            ...
            ax3.plot(self.episode_steps[i], label=f'Robot {i+1}')
        ...
        ax3.set_title('Timesteps vs Episode')
    ...
    def run(self):
        ...
        while self.step(self.timestep) != -1 and self.episode < self.max_episodes:
            ...
            if sum(self.cleaned_dirt) >= total_dirt or all(s >= self.max_steps for s in self.step_counters):
                ...
                print(f"Episode {self.episode} done. Rewards: {self.episode_rewards}, Cleaned: {self.cleaned_dirt}, Steps: {self.step_counters}")
                self.episode += 1
                self.reset_episode()
