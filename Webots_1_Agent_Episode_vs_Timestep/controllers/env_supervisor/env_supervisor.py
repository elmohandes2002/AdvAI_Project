...
class CleaningSupervisor(Supervisor):
    def __init__(self):
        ...
        self.max_steps_per_episode = 50000  # New: max steps per episode
        ...
        self.steps_history = []  # To track steps per episode
    ...
    def save_results(self):
        with open('results.csv', 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(['Episode', 'Total Reward', 'Tiles Cleaned', 'Time Steps'])
            for i in range(len(self.reward_history)):
                writer.writerow([i, self.reward_history[i], self.cleaned_history[i], self.steps_history[i]])
    ...
    def run(self):
        ...
        while self.step(self.timestep) != -1 and self.episode < self.max_episodes:
            ...
            # End episode when all dirt is cleaned OR max steps reached
            if self.cleaned_dirt >= np.sum(self.original_grid == 1) or self.step_counter >= self.max_steps_per_episode:
                self.update_policy()
                self.reward_history.append(self.episode_reward)
                self.cleaned_history.append(self.cleaned_dirt)
                self.steps_history.append(self.step_counter)
                
                print(f"Episode {self.episode} finished in {self.step_counter} steps. Total reward: {self.episode_reward:.2f}, Cleaned: {self.cleaned_dirt}/{np.sum(self.original_grid == 1)} tiles")
